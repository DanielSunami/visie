/**
 * @api {post} /pessoa Insere nova pessoa
 * @apiName Insere Pessoa
 * @apiGroup Pessoa
 *
...
function (req, res, next) {
	...
	knex.insert(req.body)
	  .into('pessoas')
	  .then(function(data) {
		res.json({ ok: true, data: { id_pessoa: data[0] } });
	}).catch(function(err) {
		console.error(err);
		res.status(500).json({ ok: false, msg: 'Erro ao inserir pessoa', cod: 6 });
	});
}
];