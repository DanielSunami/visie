<template>
	<b-modal id="pessoa-detalhes" :hide-header-close="true" :title="'Editando: ' + form.nome" @hide="reset" @shown="load" ref="modal">
		<form>
			<div class="row">
				<div class="col-lg-12">
					<b-form-group label="ID">
						<b-form-input type="text" :value="pessoaId" readonly/>
					</b-form-group>
				</div>
			</div>
			<div class="row">
		...
		<template #modal-footer="{ cancel }">
			<div class="row col-12 justify-content-end">
				...
				<div class="col-sm-12 col-lg-4 mt-sm-3 mt-lg-0">
...
export default {
	...
	methods: {
		load() {
			this.loading = true;
			client.get('/pessoa/'+this.pessoaId, (res) => {
				this.loading = false;
				if(res.ok) {
					this.form = res.data;
				} else {
		...
		submit() {
			this.enviando = true;
			client.post('/pessoa/'+this.pessoaId, this.form, (res) => {
				this.enviando = false;
				if(res.ok) {
					this.$bvToast.toast('Pessoa atualizada', {
						title: 'Sucesso!',
						variant: 'success',
						solid: true,
						toaster: 'b-toaster-bottom-center'
